# Specification Quality Checklist: リプレイ一覧
...
- サイト共通のヘッダー・フッター・配色切替は [001-home](../../001-home/spec.md) が唯一の
  記述箇所のため、本specでは扱わない。
- 限定公開のキーワードは閲覧のハードルであって、アクセス制御ではない(キーワードは
  閲覧者の手元に渡る情報から知り得る)。この前提は Assumptions に明記している。
- 追加読み込みの単位(10件)は現行の実装値をそのまま仕様として記録した。