...
//   <=\d+       … 技能値
//   [eh]        … 難易度指定。h=ハード、e=イクストリーム。"CC<=80h 【知識】" と "CC<=75 h【知識】" の
//                 両方の位置で現れる。同じ技能を難易度違いで振っても1つの技能として扱いたいので
//                 技能名から外す。直後が英数字なら難易度指定ではなく技能名の頭文字(EDU など)と
//                 みなして外さない
//   (.*?)       … 技能名。閉じ括弧が欠けていても、後置の注記があってもそのまま取る
//   \s*\(1D100  … ここから先は結果部分。\s は全角スペース(U+3000)にもマッチする
const checkRollReg =
  /^CC(-?\d+)?<=\d+\s*(?:[eh](?=[^a-z0-9]|$))?\s*(.*?)\s*\(1D100<=\d+\)/i;
